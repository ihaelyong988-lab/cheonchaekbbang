...
// 조사를 떼되, 제거가 과해 한 글자만 남으면(예: "정의"→"정") 원 토큰을 보존한다.
...
function queryIntent(input) {
  ...
  const direct = [...new Set(query.split(" ").filter(Boolean)
...
export function createQuestionSearch(books) {
  ...
  return (input, limit = 8) => {
    ...
    if (intent.query.length < 2 || (intent.direct.length === 0 && intent.anchors.length === 0)) return [];
